class_name StatusEffectDefenseUp
extends StatusEffect

@export_range(1, 3, 1) var stack_level : int = 1
@export_range(0.0, 1.0, 0.0001) var def_value_percent_increase_total : float = 0.15
...
const _SOURCE_KEY : StringName = &"status_defense_up"
...
func _sync_from_stack() -> void:
	def_value_percent_increase_total = get_percent_for_stack(stack_level)
...
func _apply_defense_modifier(actor : ActorData) -> void:
	var magnitude : float = abs(def_value_percent_increase_total)
	if magnitude <= 0.0:
		return
