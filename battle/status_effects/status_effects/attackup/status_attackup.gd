class_name StatusEffectAttackUp
extends StatusEffect

@export_range(1, 3, 1) var stack_level : int = 1
@export_range(0.0, 1.0, 0.0001) var atk_value_percent_increase_total : float = 0.15
...
const _SOURCE_KEY : StringName = &"status_attack_up"
...
func _sync_from_stack() -> void:
	atk_value_percent_increase_total = get_percent_for_stack(stack_level)
...
func _apply_attack_modifier(actor : ActorData) -> void:
	var magnitude : float = abs(atk_value_percent_increase_total)
	if magnitude <= 0.0:
		return
