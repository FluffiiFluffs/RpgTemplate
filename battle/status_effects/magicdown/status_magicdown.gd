class_name StatusEffectMagicDown
extends StatusEffect

@export_range(1, 3, 1) var stack_level : int = 1
@export_range(0.0, 1.0, 0.0001) var magic_percent_decrease_total : float = 0.15
...
const _SOURCE_KEY : StringName = &"status_magic_down"
...
func _sync_from_stack() -> void:
	magic_percent_decrease_total = get_percent_for_stack(stack_level)
...
func _apply_magic_modifier(actor : ActorData) -> void:
	var magnitude : float = abs(magic_percent_decrease_total)
	if magnitude <= 0.0:
		return
