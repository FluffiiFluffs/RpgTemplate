...
@export var spawned_all_enemies : bool = false
...
func spawn_enemy()->void:
	...
	if random_chance > enemy_spawn_chance:
		...
		var random_actor = egroup.enemies[en_index].field_scene.instantiate() #egroup will contain packedscenes, so instantiate a new packedscene
...
func if_stays_in_shape(_enemy:FieldEnemy, _rand_shape:CollisionShape2D)->void:
	if stays_in_shape:
