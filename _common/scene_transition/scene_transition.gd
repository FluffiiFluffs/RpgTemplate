...
@export var target_transition_area : StringName = &"SceneTransition"
...
@export var distance_to_spawn : int = 16
...
func on_body_entered(body : Node2D)->void:
	...
	if scene_to_load == null:
		return
	if SceneManager == null or SceneManager.main_scene == null:
		return
	...
	await SceneManager.main_scene.change_field_to(scene_to_load, target_transition_area)
