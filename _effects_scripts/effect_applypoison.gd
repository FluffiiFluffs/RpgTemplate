class_name EffectApplyPoison
extends Effect

@export_category("Poison")
## StatusEffectPoison template to instantiate (strength is authored on the .tres via exports).
@export var poison_status_id : StringName = &"veryweakpoison"
...
func apply(ctx : EffectContext, target : ActorData) -> bool:
	...
	var new_poison : StatusEffect = Registry.instantiate_status(poison_status_id)
	...
	if result.has("outcome"):
		if result["outcome"] == StatusSystem.AddStatusOutcome.BLOCKED:
			var target_name : String = target.get_display_name()
			if target_name == "":
				target_name = "Target"
		...
		if result["outcome"] == StatusSystem.AddStatusOutcome.REPLACED:
			var target_name2 : String = target.get_display_name()
			if target_name2 == "":
				target_name2 = "Target"
