class_name EffectApplyDefenseDown
extends Effect

@export_category("Defense Down")
## Strength of this cast.
## 1 adds 1 stack, 2 adds 2 stacks, 3 adds 3 stacks.
## If Defense Up is present, this removes that many stacks instead and stops at 0.
...
func apply(ctx : EffectContext, target : ActorData) -> bool:
	...
	# Tug of war: reduce Defense Up first.
	# If this cast fully removes Defense Up and still has remaining strength,
	# apply the remainder as Defense Down (crossing 0).
	...
	if existing_up != null:
		...
		if strength < up_level:
			...
			ctx.queue_battle_message("{target}'s defense up weakens.", target_battler)
			return true
		...
		# strength > up_level, remove up and carry remainder into Defense Down.
		status_system.remove_status(target_battler, existing_up)
		strength = strength - up_level
		# continue into Defense Down application using the remainder

	# No Defense Up (or remainder exists): add stacks to Defense Down (cap at 3).
	...
	if current_level >= 3:
		ctx.queue_battle_message("{target}'s defense cannot fall further.", target_battler)
		return false
	...
	if existing_down != null:
		...
		var down : StatusEffectDefenseDown = Registry.instantiate_status(&"statusdefensedown") as StatusEffectDefenseDown
		down.configure(desired_level, duration_turns)
		status_system.add_status(target_battler, down, caster_battler)

	if desired_level == 1:
		ctx.queue_battle_message("{target}'s defense falls!", target_battler)
	elif desired_level == 2:
		ctx.queue_battle_message("{target}'s defense falls further!", target_battler)
	else:
		ctx.queue_battle_message("{target}'s defense falls to its limit!", target_battler)
